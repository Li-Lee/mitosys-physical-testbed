...
function getTopicFor (device, property, type) {
    ...
    if (device === 'Shades' && property === 'windowShade') {
        property = 'bool_Closed';
    }

    if (device === 'Thermostat' && property === 'thermostatSetpoint') {
        property = 'int_Temp';
    }

    if (property === 'lock') {
        property = 'bool_Locked';
    }

    if (property === 'alarm') {
        property = 'str_Alarm_mode';
    }

    if (property === 'motion') {
        property = 'bool_motion_sensed';
    }
...
function parseMQTTMessage (topic, message) {
    ...
    if (pieces[0] === 'Shades') {
        if (pieces[1] === 'bool_Closed') {
            pieces[1] = 'windowShade';
        }
    ...
    if (pieces[0] === 'Thermostat' && pieces[1] === 'int_Temp') {
        pieces[1] = 'thermostatSetpoint';
    }

    if (pieces[1] === 'bool_Locked') {
        pieces[1] = 'lock';
    }

    if (pieces[1] === 'str_Alarm_mode') {
        pieces[1] = 'alarm';
    }

    if (pieces[1] === 'bool_motion_sensed') {
        pieces[1] = 'motion';
    }

    if (pieces[0] === 'OpenCloseSensorState' && pieces[1] === 'bool_Closed') {
        pieces[1] = 'contact';
    }

    if (pieces[1] === 'bool_Smoke_Detected') {
        pieces[1] = 'smoke';
    }

    if (pieces[1] === 'int_Temp') {
        pieces[1] = 'temperature';
    }

    if (pieces[0] === 'DoorLock') {
        if (contents === 'true') {
            contents = 'locked';
        }
